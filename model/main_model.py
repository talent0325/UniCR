...
from .models import New_Audio_Guided_Attention, Co_Guided_Attention, Only_Visual_Guided_Attention
...
class CAS_Module(nn.Module):
    def __init__(self, d_model, num_class=101):
...
class SupvLocalizeModule(nn.Module):
    ...
    def forward(self, fused_content):
        ...
        # class_logits = self.event_classifier(max_fused_content)  # 样本级别
        class_logits = self.event_classifier(fused_content)  # 帧级别
...
class weak_main_model(nn.Module):
    def __init__(self, config):
        ...
        self.CAS_model = CAS_Module(d_model=self.d_model, num_class=101)
...
class supv_main_model(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.psai = self.config['psai']
        self.beta = self.config["beta"]
        self.guide = self.config['guide']
        self.video_input_dim = self.config['video_inputdim']
        self.audio_input_dim = self.config['audio_inputdim']
    ...
    def forward(self, audio_feature, visual_feature):
        # [batch, 10, 128]   visual_feature: [batch, 10, 7, 7,512]
        ...
        # MARK: spatial-channel attention
        if self.guide == 'co_guide':
            visual_feature, audio_rnn_input = self.spatial_channel_att(visual_feature, audio_rnn_input)
        elif self.guide == 'audio':
            visual_feature = self.spatial_channel_att(visual_feature, audio_rnn_input)
        elif self.guide == 'video':
            visual_feature, audio_rnn_input = self.spatial_channel_att(visual_feature, audio_rnn_input)
        visual_rnn_input = visual_feature
        ...
        video_query_output = video_query_output.transpose(0, 1)
        audio_query_output = audio_query_output.transpose(0, 1)
        Multimodal_feature = (video_query_output + audio_query_output) / 2
        return audio_query_output, video_query_output, Multimodal_feature
